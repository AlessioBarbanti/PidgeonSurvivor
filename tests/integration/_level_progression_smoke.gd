...
func _run() -> void:
	...
	await process_frame
	_validate_curve()
	await _validate_level_queue_and_overflow()
	await _validate_composed_level_flow()
	await _finish()


func _validate_curve() -> void:
	...
	curve.experience_growth_per_level = -5
	_expect(
		curve.get_experience_required(1) == 1
		and curve.get_experience_required(50) == 1,
		"La curva XP deve difendersi da valori runtime non validi."
	)


func _validate_level_queue_and_overflow() -> void:
	...
	_expect(controller.start_run(8001), "La fixture B08 deve avviare la run.")
	_expect(
		experience.level == 1
	...
	_expect(experience.add_experience(9), "RUNNING deve accettare XP sotto soglia.")
	_expect(
		experience.level == 1
		and experience.experience_current == 9
		and experience.experience_total == 9
		and controller.is_running(),
		"Gli XP sotto soglia devono restare nella barra senza aprire LEVEL_UP."
	)

	_expect(experience.add_experience(1), "La soglia esatta deve essere accettata.")
	_expect(
		experience.level == 2
		and experience.experience_current == 0
	...
	_expect(
		...
		"LEVEL_UP deve bloccare accrediti XP concorrenti."
	)
	_expect(experience.complete_level_up(), "La prima scelta deve poter essere completata.")
	_expect(
	...
	_expect(
		...
		"Un accredito grande deve poter attraversare più soglie."
	)
	_expect(
		experience.level == 4
	...
	_expect(experience.complete_level_up(), "La prima scelta multipla deve chiudersi.")
	_expect(
		controller.get_state() == RunController.RunState.LEVEL_UP
		and paused
	...
	_expect(experience.complete_level_up(), "La seconda scelta multipla deve chiudersi.")
	_expect(
		controller.is_running()
		and not paused
		and experience.pending_level_ups == 0,
	...
	_expect(
		_queued_levels == [2, 3, 4]
		and _started_levels == [2, 3, 4]
		and _completed_levels == [2, 3, 4],
		"Ogni livello guadagnato deve accodare, presentare e completare una scelta."
	)

	_expect(
		experience.add_experience(20)
		and experience.level == 5
		and experience.experience_current == 0,
		"L'overflow precedente deve contribuire alla soglia successiva."
	)
	_expect(
		controller.request_defeat(),
		"Un terminale deve poter interrompere LEVEL_UP con priorità."
	)
	_expect(
		not experience.complete_level_up()
		and experience.pending_level_ups == 1,
		"Una scelta interrotta dal terminale non deve essere consumata."
	)
	_expect(controller.restart_run(8002), "Il terminale deve accettare il restart.")
	_expect(
		controller.is_running()
...
func _validate_composed_level_flow() -> void:
	...
	_expect(controller != null, "La scena B08 deve contenere RunController.")
	_expect(experience != null, "La scena B08 deve contenere ExperienceSystem.")
	...
	_expect(experience.experience_curve != null, "La scena B08 deve caricare la curva XP dati.")
	_expect(
	...
	_expect(experience.complete_level_up(), "La scena deve consumare la scelta livello 2.")
	_expect(experience.complete_level_up(), "La scena deve consumare la scelta livello 3.")
	_expect(experience.complete_level_up(), "La scena deve consumare la scelta livello 4.")
	_expect(
		controller.is_running() and experience.pending_level_ups == 0,
		"La scena composta deve riprendere dopo tutte le scelte."
	)
...
func _finish() -> void:
	...
	if _failures.is_empty():
		print("B08_LEVEL_PROGRESSION_SMOKE_OK")
		quit(0)
		return

	for failure in _failures:
		push_error(failure)
	printerr("B08_LEVEL_PROGRESSION_SMOKE_FAIL count=%d" % _failures.size())
	quit(1)
