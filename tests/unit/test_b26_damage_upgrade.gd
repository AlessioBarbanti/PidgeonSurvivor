...
func test_damage_upgrade() -> void:
	...
	# PS-012/PS-077: Gossip e Birra sono Specialità di Barb, bloccate
	# all'inizio della run. Vanno sbloccate da due ricompense Boss distinte,
	# non pescate dal level-up normale.
	...
	service.queue_barb_reward()
	assert_true(
		service.select_barb_speciality(BEER.id),
		"Birra deve essere sbloccabile come Specialità di Barb prima della pesca B26."
	)
