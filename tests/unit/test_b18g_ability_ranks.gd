...
func test_rank_catalog_and_flow() -> void:
	...
	assert_true(
		controller != null and abilities != null and ability != null and upgrades != null
		and service != null and experience != null,
		"La scena composta B18G deve esporre tutti i servizi richiesti."
	)
	if (
		controller == null
		or abilities == null
	...
	_assert_declared_snapshots(abilities, upgrades)
	var magno_id := &"magno_earthquake_shockwave"
	var magno_card_id := UpgradeDefinition.get_ability_rank_upgrade_id(magno_id)
	assert_eq(
		service.get_equipped_ability_id(), magno_id, "La run iniziale deve filtrare le carte rank su Magno."
	)
	assert_eq(service.get_rank(magno_card_id), 1, "L'abilita equipaggiata deve partire al rank 1.")
	assert_eq(ability.get_ability_rank(), 1, "AbilityController deve partire dal profilo rank 1.")

	for target_rank in range(2, 4):
		assert_true(
			_open_next_level(experience), "La fixture deve aprire il level-up per il rank %d." % target_rank
		)
		...
		if rank_card == null or not service.select_upgrade(rank_card.id):
			assert_true(false, "La selezione rank %d deve essere atomica." % target_rank)
			break
		assert_eq(
			service.get_rank(magno_card_id), target_rank, "UpgradeService deve registrare il rank %d." % target_rank
		)
		assert_eq(
			ability.get_ability_rank(), target_rank, "AbilityController deve applicare il rank %d." % target_rank
		)
	...
	assert_true(ability.try_activate(), "Il rank 3 deve produrre uno snapshot di attivazione.")
	...
	assert_true(
		rank_four_card != null and service.select_upgrade(rank_four_card.id),
		"La carta rank 4 deve essere selezionabile durante il cooldown."
	)
	assert_eq(ability.get_ability_rank(), 4, "Il nuovo rank deve valere per la prossima attivazione.")
	assert_almost_eq(
		ability.get_activation_definition().cooldown_seconds, 7.0, FLOAT_TOLERANCE,
	...
	assert_true(ability.is_cooldown_ready(), "Il cooldown fotografato deve completarsi nel clock RUNNING.")
	assert_almost_eq(
		ability.get_cooldown_total(), 7.0, FLOAT_TOLERANCE, "Da pronto, il cooldown deve mostrare il nuovo rank."
	)

	assert_true(_open_next_level(experience), "La fixture deve aprire il level-up rank 5.")
	var rank_five_card := _reroll_until_rank_card(service, magno_id)
	assert_true(
		rank_five_card != null and service.select_upgrade(rank_five_card.id),
		"La carta rank 5 deve essere selezionabile."
	)
	assert_true(
		service.get_rank(magno_card_id) == 5 and ability.get_ability_rank() == 5,
		"Servizio e controller devono convergere al rank 5."
	)

	assert_true(_open_next_level(experience), "La fixture deve aprire un'offerta dopo il cap.")
	for _attempt in 12:
		var offer := service.generate_offer(experience.get_active_level_up_level())
		assert_true(_has_unique_ids(offer), "Ogni offerta B18G deve avere ID unici.")
		assert_true(
			magno_card_id not in _offer_ids(offer), "Al rank 5 la carta abilita non deve piu essere eleggibile."
		)

	controller.prepare_restart()
	assert_true(service.get_ranks().is_empty(), "Il restart deve rimuovere tutti i rank della run.")
	assert_true(
		service.get_rank(magno_card_id) == 1 and ability.get_ability_rank() == 1,
		"La seconda run deve ripartire dal rank 1."
	)
	assert_true(
		movement_slice.select_friend_for_next_run(&"bea"), "In BOOT deve essere possibile cambiare profilo."
	)
	var bea_id := &"bea_fire_z_trail"
	var bea_card_id := UpgradeDefinition.get_ability_rank_upgrade_id(bea_id)
	assert_eq(service.get_equipped_ability_id(), bea_id, "Il filtro carta deve seguire il nuovo personaggio.")
	assert_true(
		service.get_rank(bea_card_id) == 1 and ability.get_ability_rank() == 1,
		"Bea deve iniziare al rank 1 senza eredita da Magno."
	)
	assert_true(movement_slice.start_selected_run(1807), "La seconda run B18G deve avviarsi.")
	assert_true(_open_next_level(experience), "La seconda run deve generare una nuova offerta.")
	var bea_card := _reroll_until_rank_card(service, bea_id)
	assert_not_null(bea_card, "La seconda run deve offrire soltanto la carta dell'abilita equipaggiata.")
	if bea_card != null:
		assert_eq(bea_card.id, bea_card_id, "L'ID carta deve essere derivato dall'ability ID selezionato.")

	controller.prepare_restart()


func _assert_declared_snapshots(abilities: AbilityEffectRegistry, upgrades: UpgradeRegistry) -> void:
	assert_eq(abilities.get_definitions().size(), 8, "B18G richiede otto AbilityDefinition.")
	...
	for ability_id_value: Variant in EXPECTED_RANKS:
		...
		assert_true(
			definition != null and definition.is_valid(), "AbilityDefinition non valida: %s." % ability_id
		)
		if definition == null:
			continue
		assert_eq(
			definition.rank_snapshots.size(), 5, "%s deve dichiarare cinque snapshot." % ability_id
		)
		...
		for rank_index in 5:
			...
			assert_true(
				snapshot != null and resolved != null, "%s rank %d deve essere risolvibile." % [ability_id, rank]
			)
			if snapshot == null or resolved == null:
				continue
			assert_almost_eq(
				resolved.cooldown_seconds, float(expected[0]), FLOAT_TOLERANCE, "%s R%d cooldown." % [ability_id, rank]
			)
			assert_almost_eq(
				resolved.duration_seconds, float(expected[1]), FLOAT_TOLERANCE, "%s R%d durata." % [ability_id, rank]
			)
			assert_almost_eq(
				resolved.area_radius, float(expected[2]), FLOAT_TOLERANCE, "%s R%d area." % [ability_id, rank]
			)
			assert_almost_eq(
				resolved.damage, float(expected[3]), FLOAT_TOLERANCE, "%s R%d danno." % [ability_id, rank]
			)
			assert_eq(
				snapshot.effect_parameters.keys().size(), baseline_keys.size(),
				"%s R%d deve essere uno snapshot completo." % [ability_id, rank]
			)
			for baseline_key: Variant in baseline_keys:
				assert_true(
					snapshot.effect_parameters.has(baseline_key),
					"%s R%d non deve perdere %s." % [ability_id, rank, baseline_key]
				)
		...
		assert_true(
			card != null and card.is_ability_rank_definition(), "Carta rank non valida per %s." % ability_id
		)
		if card != null:
			rank_card_count += 1
	assert_eq(rank_card_count, 8, "Il catalogo deve contenere otto carte rank distinte.")
...
func _reroll_until_rank_card(service: UpgradeService, ability_id: StringName) -> UpgradeDefinition:
	...
	for _attempt in 100:
		...
		assert_true(_has_unique_ids(offer), "La carta rank non puo duplicarsi nella stessa offerta.")
		for definition in offer:
			if definition.effect_id == UpgradeEffectRegistry.ABILITY_RANK:
				...
				assert_eq(
					offered_ability_id, ability_id, "Le carte di abilita non equipaggiate devono essere escluse."
				)
