...
- [ ] La riga statistiche del recap (`EndScreen`, sia sconfitta sia
      ...
      applicata in PS-166.
- [ ] Il conteggio include ogni nemico (`BaseEnemy` e sue sottoclassi,
      ...
      sono uccisioni.
- [ ] Il conteggio riparte da zero a ogni nuovo avvio di run
      (`RunController.run_started`), stesso momento in cui si azzera
      `_defeated_boss_count` in `MovementSlice`.
- [ ] Il conteggio compare correttamente sia dopo una sconfitta sia dopo
...
- [ ] Controllo percettivo richiesto: sì — verificare che la riga
      ...
      numeri a più cifre, sia in landscape stretto che largo
...
- [ ] Nessuno atteso: statistica di sola UI, non cambia un contratto di
