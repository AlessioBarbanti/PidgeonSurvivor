...
palette, costume, acconciatura, accessori e grammatica pixel-art. Le reference
visive riservate del cast vivono sotto
`assets/art/characters/references/<id>/`, un albero pensato per asset, mentre
sono materiale sorgente e di provenienza, non un asset da derivare.

## Comportamento atteso

Ogni personaggio giocabile ha un documento di direzione visuale in `docs/`,
verificabile contro i master correnti e i manifest, non inventato. Le reference
visive riservate del cast vivono accanto a quella documentazione invece che sotto
...
di ispezionare i fratelli visivi correnti.
...
- [x] Esiste un file Markdown per ciascuno degli otto personaggi giocabili
      ...
      master/derivati correnti ed eventuale reference visiva riservata associata
...
- [x] Ogni fatto riportato nei nuovi file è tracciabile a
      ...
      percorsi correnti dei file; nessuna provenienza o approvazione
      viene inventata.
- [x] Il contenuto di `assets/art/characters/references/` (file sorgente e
      `README.md`) è spostato sotto `docs/characters/references/<id>/`, senza
      alterare i byte dei file.
...
- [x] `.agents/skills/game-art-designer/SKILL.md` dichiara esplicitamente che
      ...
      ispezionare i fratelli visivi correnti né gli altri vincoli già in vigore
      (provenienza verificata, ruolo esplicito dei reference, niente asset
      generato in anticipo).
- [x] `.agents/skills/game-art-designer/references/imagegen-reference-policy.md`
      indica il nuovo percorso delle reference visive riservate del
      cast.
...
- Spostamento: `assets/art/characters/references/**` →
  `docs/characters/references/**` (file sorgente, `README.md`, `.gdignore`).
...
- Correzione minore: `docs/visual-audio-identity.md` viene riallineato
  all'identity pass del 28/08/2026 già registrato in
  `assets/art/characters/ASSET-MANIFEST.md`.
...
- **2026-09-02 — Le reference visive riservate traslocano sotto `docs/`.**
  Sono materiale sorgente e di provenienza, mai runtime, non un asset da
  derivare: la loro casa naturale è
...
- **2026-09-02 — I nuovi file sono un aiuto, non una fonte sostitutiva.**
  ...
  l'ispezione dei fratelli visivi correnti; i documenti per personaggio
  ...
  correnti.
- **2026-09-02 — Nomi tecnici neutrali.** I file sorgente restano identificati
  solo con l'id del personaggio in gioco (`alea`, `aleo`, …), coerentemente con
  il trattamento già in uso nel manifest e nell'archivio storico.
