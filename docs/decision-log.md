...
| UI-004 | Baseline operativa | B18B riunisce ritratto, livello, vita, timer e pausa in una fascia alta `64` unità logiche, con linea XP da `8`; la prima card abilità `246×94` viene sostituita dal controllo icon-only B18K, mentre pausa resta almeno `44×44` | Il campo di gioco guadagna priorità senza restringere hit target; Boss UI, overlay e terminali mantengono i propri layer e la safe area dinamica |
...
| UI-007 | Confermata | B18Q sottrae dal rect autorevole di `ArenaLayout` la fascia superiore occupata da vita, timer, pausa e XP | Player, drop, spawn, Boss, telegraph e target non possono sovrapporsi al HUD; tutti i consumatori usano lo stesso playfield responsive senza coordinate fisse |
