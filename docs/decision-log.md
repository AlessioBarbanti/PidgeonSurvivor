...
| VISUAL-003 | Baseline operativa | B18C usa per ogni `FriendDefinition` una posa laterale destra e quattro fasi di camminata; dove il foglio CC0 offre più ritagli li alterna, mentre Lollo, Migi e Marghe animano l'unica posa valida con bob/inclinazione. Il Player ribalta gli sprite per la sinistra, non disegna più corpo circolare o cannoncino e conserva l'ultima direzione orizzontale non nulla | Il neutro e il movimento verticale non cancellano la direzione; pausa e focus azzerano il movimento senza cambiarla, una nuova run riparte verso destra e le abilità possono leggerla tramite `Player.get_facing_direction()` |
