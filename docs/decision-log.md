...
| UI-009 | In verifica | B18W adotta una grammatica pixel-fantasy unica: fondale incorniciato, card squadrate, mini-card complete, frecce metalliche/oro, Back discreto e kit laterale con icone e titoli dorati equivalenti per passiva e attiva; ogni profilo usa la propria icona passiva raster elaborata, senza fallback al ritratto; l'unico CTA è una placca ImageGen senza testo con copy Godot dinamica | La placca resta centrata, larga al massimo il `55%` del pannello e separata dalla cornice. Il sottotitolo è rimosso; `KIT DI <NOME>` largo `380` lega il pannello al profilo, mentre nome e ruolo condividono un blocco a distanza `6–12` e il CTA lo segue entro `34`, senza vuoto centrale. Ciano identifica soltanto selezione/focus, oro e arancione la gerarchia importante; 16:9, 20:9 e 4:3 conservano le regole `BOOT` di UI-008 |
...
| BOSS-004 | Pianificata post-release | B22 rende il piccione speciale B18H il Boss baseline e usa il seed della run per una probabilità dati iniziale del `25%` di sostituirlo con un `Evil <Nome>` casuale | La prima variante Evil riusa sprite Player, hitbox, statistiche e due pattern del piccione Boss con palette viola scura e accenti magenta; le abilità dei profili restano esplicitamente fuori B22 |
| MODE-001 | Pianificata post-release | B23 aggiunge **Difesa Grigliata** accanto a Sopravvivenza: una griglia con carne centrale possiede salute e può ricevere danno dai nemici | La run termina se muore Player o obiettivo; selezione modalità, Boss, pausa, lifecycle, restart, seed e cleanup restano autorevoli in `RunController` e non contaminano l'altra modalità |
